        ...
        <div className="absolute top-14 right-4 w-96 bg-slate-900/98 border border-white/10 rounded-2xl shadow-2xl p-4 z-50 animate-in fade-in slide-in-from-top-3 duration-150 backdrop-blur-xl">
            <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-white flex items-center gap-2">
                    <Clipboard size={16} /> Clipboard History
                </h3>
                <button
                    onClick={onClose}
            ...
            <div className="max-h-96 overflow-y-auto space-y-2">
                {history.map(item => (
                    <div key={item.id} className="bg-slate-800 rounded-xl p-3 group">
                        ...
                        <pre className="text-xs text-slate-200 whitespace-pre-wrap break-words font-sans line-clamp-3">
                            {item.content}
                        </pre>
                    </div>
                ))}
                {history.length === 0 && (
                    <div className="text-center text-slate-500 py-8">Clipboard is empty</div>
                )}
            </div>
...
export const ClipboardPanel: React.FC<{ onClose: () => void }> = ({ onClose }) => (
