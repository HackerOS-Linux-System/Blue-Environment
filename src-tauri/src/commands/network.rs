...
/// Last time a background Wi-Fi rescan was requested — see below.
static LAST_WIFI_RESCAN: once_cell::sync::Lazy<std::sync::Mutex<Option<std::time::Instant>>> =
    once_cell::sync::Lazy::new(|| std::sync::Mutex::new(None));

/// Lists visible Wi-Fi networks.
///
/// This used to be a plain (synchronous) command that ran
/// `nmcli dev wifi rescan` and *waited for it* — on the UI thread. A radio
/// rescan takes seconds, so opening the Wi-Fi list froze the whole shell
/// (~3 s) and the Control Center's 4 s polling repeated it. Now:
///  * the command is `async` and does its work on a blocking thread;
///  * a rescan is only *requested* (fire-and-forget, at most every 10 s);
///  * the list itself is read with `--rescan no`, i.e. instantly from
///    NetworkManager's cache — fresh results show up on the next poll.
#[tauri::command]
pub async fn get_wifi_networks_real() -> Result<Vec<WifiNetwork>, String> {
    tokio::task::spawn_blocking(|| -> Vec<WifiNetwork> {
        {
            let mut last = LAST_WIFI_RESCAN.lock().unwrap();
            let due = last.map(|t| t.elapsed() > std::time::Duration::from_secs(10)).unwrap_or(true);
            if due {
        ...
        if let Ok(o) = Command::new("nmcli")
            .args(["-t", "-f", "IN-USE,BSSID,SSID,MODE,CHAN,FREQ,RATE,SIGNAL,BARS,SECURITY", "dev", "wifi", "list", "--rescan", "no"])
            .output()
        {
...
#[tauri::command]
pub async fn connect_wifi_real(ssid: String, password: String) -> Result<String, String> {
...
#[tauri::command]
pub async fn get_wifi_radio_enabled() -> Result<bool, String> {
...
#[tauri::command]
pub async fn get_saved_wifi_connections() -> Result<Vec<String>, String> {
...
#[tauri::command]
pub async fn forget_wifi_network(ssid: String) -> Result<(), String> {
    tokio::task::spawn_blocking(move || -> Result<(), String> {
...
#[tauri::command]
pub async fn rename_saved_wifi_connection(old_name: String, new_name: String) -> Result<(), String> {
    tokio::task::spawn_blocking(move || -> Result<(), String> {
...
#[tauri::command]
pub async fn update_saved_wifi_password(name: String, password: String) -> Result<(), String> {
    tokio::task::spawn_blocking(move || -> Result<(), String> {
...
#[tauri::command]
pub async fn connect_saved_wifi_connection(name: String) -> Result<(), String> {
    tokio::task::spawn_blocking(move || -> Result<(), String> {
...
#[tauri::command]
pub async fn get_bluetooth_devices_real() -> Result<Vec<BluetoothDevice>, String> {
    tokio::task::spawn_blocking(move || -> Vec<BluetoothDevice> {
...
#[tauri::command]
pub async fn bluetooth_connect(mac: String) -> Result<(), String> {
    tokio::task::spawn_blocking(move || -> Result<(), String> {
...
#[tauri::command]
pub async fn bluetooth_disconnect(mac: String) -> Result<(), String> {
    tokio::task::spawn_blocking(move || -> Result<(), String> {
...
#[tauri::command]
pub async fn bluetooth_pair(mac: String) -> Result<(), String> {
    tokio::task::spawn_blocking(move || -> Result<(), String> {
...
#[tauri::command]
pub async fn bluetooth_forget(mac: String) -> Result<(), String> {
    tokio::task::spawn_blocking(move || -> Result<(), String> {
...
#[tauri::command]
pub async fn get_bluetooth_rssi(mac: String) -> Result<Option<i32>, String> {
...
/// w Blue Environment".
#[tauri::command]
pub async fn has_cellular_modem() -> Result<bool, String> {
    tokio::task::spawn_blocking(move || -> bool {
...
#[tauri::command]
pub async fn get_cellular_status() -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub async fn set_cellular_enabled(enabled: bool) -> Result<(), String> {
    tokio::task::spawn_blocking(move || -> Result<(), String> {
        let list = Command::new("mmcli").arg("-L").output().map_err(|e| e.to_string())?;
        let list_text = String::from_utf8_lossy(&list.stdout);
        let modem_path = list_text
